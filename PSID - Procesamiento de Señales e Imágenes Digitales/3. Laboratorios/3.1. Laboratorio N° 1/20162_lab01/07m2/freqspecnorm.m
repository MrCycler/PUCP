...
function [ X_v, f_v]= freqspec_norm( x_v)

N = length(x_v);
f_v= 2* pi* ( 0: ( N- 1))/ N;
f_v= fftshift( f_v);
f_v= unwrap( f_v - 2*pi);
X_v= abs(fftshift( fft( x_v, N)));

fig_freqspec= figure;
figure( fig_freqspec); plot( f_v, X_v/max(X_v));
xlabel( 'Frecuencia Normalizada (rad)'); ylabel( 'Magnitud');
title( 'Espectro de Frecuencia Normalizado X_v/max(X_v)');